"""Integration surface for the SEO Agent (Phase 4).

Two independent entry points, mirroring website_extraction's precedent:
  ...
  - stream_run(): live per-node progress via an async generator, for the SSE
    endpoint — each of the 24 analyzer nodes yields a `node` event the
...
from .data import SeoAgentData
from .graph import SEOAnalysisGraph
from .schema import DIMENSION_NAMES
from .nodes._common import dimension_result_key
...
def _initial_state(analysis_id: str, tenant_id: str, context_id: str, extraction_id: str, extraction_data: dict) -> SEOAnalysisState:
    state: dict = {
    ...
    for dimension in DIMENSION_NAMES:
        state[dimension_result_key(dimension)] = None
...
async def _run_and_track(initial_state: SEOAnalysisState) -> AsyncIterator[tuple[str | None, dict]]:
    ...
    async for step in SEOAnalysisGraph.astream(initial_state, stream_mode="updates"):
...
async def run_analysis(analysis_id: str, tenant_id: str, context_id: str, extraction_id: str, extraction_data: dict) -> dict:
    ...
    try:
        ...
        logger.exception("seo_agent.start_update_failed analysis_id=%s", analysis_id)
    ...
    try:
        ...
        logger.exception("seo_agent.pipeline_crashed analysis_id=%s", analysis_id)
    ...
    try:
        ...
        logger.exception("seo_agent.final_persist_failed analysis_id=%s", analysis_id)
    return final_state
...
async def stream_run(analysis_id: str, tenant_id: str, context_id: str, extraction_id: str, extraction_data: dict) -> AsyncIterator[tuple]:
    ...
    try:
        ...
        logger.exception("seo_agent.stream_pipeline_crashed analysis_id=%s", analysis_id)
    ...
    try:
        ...
        logger.exception("seo_agent.stream_persist_failed analysis_id=%s", analysis_id)
