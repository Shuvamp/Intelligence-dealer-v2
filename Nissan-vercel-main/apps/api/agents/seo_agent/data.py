"""Data access for the SEO Agent over Supabase/PostgREST.

Reads website_extractions read-only via its own duplicated query (mirrors
company_summary/data.py's own read-only duplicate) rather than importing
agents.website_extraction — preserves the per-phase decoupling convention.
Owns full CRUD on seo_analyses.
"""
...
class SeoAgentData:
    ...
    async def insert_analysis(self, row: dict) -> dict | None:
        ...
        async with httpx.AsyncClient(base_url=SUPABASE_URL, timeout=15) as c:
            r = await c.post("/rest/v1/seo_analyses", json=payload, headers=_headers())
            r.raise_for_status()
            rows = r.json()
    ...
    async def list_analyses(
        ...
        async with httpx.AsyncClient(base_url=SUPABASE_URL, timeout=15) as c:
            r = await c.get("/rest/v1/seo_analyses", params=params, headers=_headers())
            r.raise_for_status()
            return r.json()
