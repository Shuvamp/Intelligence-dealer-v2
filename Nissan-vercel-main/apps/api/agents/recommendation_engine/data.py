"""Data access for the Recommendation Engine over Supabase/PostgREST.

Reads website_extractions, seo_analyses, and aeo_analyses read-only via its
own duplicated queries (mirrors seo_agent/data.py's and aeo_agent/data.py's
own read-only duplicates) rather than importing agents.website_extraction /
agents.seo_agent / agents.aeo_agent — preserves the per-phase decoupling
convention. Owns full CRUD on recommendation_reports.
"""
...
class RecommendationEngineData:
    ...
    async def get_latest_ready_seo_analysis(self, extraction_id: str, tenant_id: str) -> dict | None:
        async with httpx.AsyncClient(base_url=SUPABASE_URL, timeout=15) as c:
            r = await c.get(
    ...
    async def get_latest_ready_aeo_analysis(self, extraction_id: str, tenant_id: str) -> dict | None:
        async with httpx.AsyncClient(base_url=SUPABASE_URL, timeout=15) as c:
            r = await c.get(
    ...
    async def insert_report(self, row: dict) -> dict | None:
        ...
        async with httpx.AsyncClient(base_url=SUPABASE_URL, timeout=15) as c:
            r = await c.post("/rest/v1/recommendation_reports", json=payload, headers=_headers())
            r.raise_for_status()
            rows = r.json()
    ...
    async def list_reports(
        ...
        async with httpx.AsyncClient(base_url=SUPABASE_URL, timeout=15) as c:
            r = await c.get("/rest/v1/recommendation_reports", params=params, headers=_headers())
            r.raise_for_status()
            return r.json()
