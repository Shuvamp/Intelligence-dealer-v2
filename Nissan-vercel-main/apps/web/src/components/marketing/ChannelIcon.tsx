...
type ChannelKey =
  | 'instagram' | 'facebook' | 'linkedin' | 'google_business' | 'whatsapp'
  | 'x' | 'youtube' | 'telegram' | 'threads'
...
export function ChannelIcon({
  ...
  switch (channel as ChannelKey) {
    ...
    case 'x':
      return (
        <div className={tile} style={{ background: '#000000' }}>
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="#fff" aria-hidden>
            <path d="M18.24 2.25h3.31l-7.23 8.26 8.5 11.24h-6.66l-5.21-6.82-5.97 6.82H1.66l7.73-8.84L1.25 2.25h6.83l4.71 6.23 5.45-6.23Zm-1.16 17.52h1.83L7.02 4.13H5.05l12.03 15.64Z" />
          </svg>
        </div>
      )
    ...
    case 'youtube':
      return (
        <div className={tile} style={{ background: '#FF0000' }}>
          <svg viewBox="0 0 24 24" className="h-6 w-6" fill="#fff" aria-hidden>
            <path d="M23.5 6.5a3 3 0 0 0-2.11-2.12C19.5 3.85 12 3.85 12 3.85s-7.5 0-9.39.53A3 3 0 0 0 .5 6.5 31.4 31.4 0 0 0 0 12a31.4 31.4 0 0 0 .5 5.5 3 3 0 0 0 2.11 2.12c1.89.53 9.39.53 9.39.53s7.5 0 9.39-.53a3 3 0 0 0 2.11-2.12A31.4 31.4 0 0 0 24 12a31.4 31.4 0 0 0-.5-5.5ZM9.6 15.6V8.4l6.2 3.6-6.2 3.6Z" />
          </svg>
        </div>
      )
    ...
    case 'telegram':
      return (
        <div className={tile} style={{ background: '#229ED9' }}>
          <svg viewBox="0 0 24 24" className="h-6 w-6" fill="#fff" aria-hidden>
            <path d="M21.94 4.6 18.6 20.3c-.25 1.1-.9 1.38-1.83.86l-5.05-3.72-2.44 2.35c-.27.27-.5.5-1 .5l.36-5.13 9.34-8.44c.4-.36-.09-.56-.63-.2L5.55 13.2.6 11.65c-1.08-.34-1.1-1.08.22-1.6L20.55 2.4c.9-.34 1.68.2 1.39 2.2Z" />
          </svg>
        </div>
      )
    ...
    case 'threads':
      return (
        <div className={tile} style={{ background: '#000000' }}>
          <svg viewBox="0 0 24 24" className="h-6 w-6" fill="#fff" aria-hidden>
            <path d="M16.5 11.2c-.08-.04-.17-.08-.25-.11-.15-2.75-1.65-4.33-4.18-4.34h-.03c-1.51 0-2.77.65-3.54 1.82l1.39.95c.58-.87 1.48-1.06 2.15-1.06h.02c.84 0 1.47.25 1.88.72.3.35.5.83.6 1.44-.72-.12-1.5-.16-2.32-.11-2.32.13-3.81 1.49-3.71 3.37.05.96.53 1.78 1.34 2.31.69.45 1.57.67 2.49.62 1.22-.07 2.17-.53 2.84-1.38.51-.65.83-1.49.98-2.55.6.36 1.05.84 1.29 1.42.42.98.44 2.6-.87 3.9-1.15 1.15-2.53 1.64-4.62 1.66-2.31-.02-4.06-.76-5.2-2.21C4.19 15.9 3.63 14 3.6 12c.03-2 .59-3.9 1.66-5.26C6.4 5.29 8.15 4.55 10.46 4.53c2.33.02 4.1.76 5.27 2.22.57.72 1 1.62 1.29 2.68l1.62-.43c-.34-1.3-.88-2.43-1.63-3.37C15.06 3.73 12.86 2.8 10.05 2.78h-.01c-2.8.02-4.98.96-6.47 2.79C1.86 8.61 1.6 10.75 1.6 12v.01c0 1.25.26 3.39 1.97 5.42 1.49 1.83 3.67 2.77 6.47 2.79h.01c2.49-.02 4.24-.67 5.68-2.11 1.89-1.88 1.83-4.24 1.21-5.69-.45-1.04-1.3-1.89-2.45-2.46Zm-4.4 4.15c-1.02.06-2.08-.4-2.13-1.36-.04-.71.51-1.51 2.19-1.6.19-.02.38-.02.57-.02.61 0 1.18.06 1.7.17-.19 2.42-1.33 2.75-2.33 2.81Z" />
          </svg>
        </div>
      )
