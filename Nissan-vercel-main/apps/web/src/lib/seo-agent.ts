/**
 * SEO Agent API client — Server-side functions (Phase 4).
 * See docs/planner/04_SEO_AGENT.md.
 */
...
export interface SeoDimensionResult {
  dimension: string
  status: SeoStatus
  recommendations: Array<SeoRecommendation>
}
...
export interface SeoAnalysisData {
  dimensions: Array<SeoDimensionResult>
  summary: SeoSummary
}
...
export interface AnalysisResult {
  ...
  analysis_data: SeoAnalysisData | Record<string, any> | null
...
export const createAnalysis = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<AnalysisResult> => {
    ...
    const res = await fetch(`${AGENT_BASE}/seo-agent/analyses`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tenant_id, extraction_id: data.extraction_id }),
    })
    if (!res.ok) {
      const err = (await res.json().catch(() => ({ detail: 'Failed to start SEO analysis' }))) as { detail?: string }
      throw new Error(typeof err.detail === 'string' ? err.detail : 'Failed to start SEO analysis')
    }
...
export const getAnalysis = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<AnalysisResult | null> => {
    ...
    const res = await fetch(`${AGENT_BASE}/seo-agent/analyses/${data.analysis_id}?tenant_id=${tenant_id}`)
...
export const listAnalyses = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<Array<AnalysisResult>> => {
    ...
    const res = await fetch(`${AGENT_BASE}/seo-agent/analyses?${params}`)
    if (!res.ok) return []
